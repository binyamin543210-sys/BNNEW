// BNAPP Calendar – Hebrew/Gregorian, Shabbat, Holidays, Firebase sync, global city selection, Waze, weather
...
// Geonames username
const GEONAMES_USERNAME = "binyamin543210";
...
async function searchCityGlobal(query) {
  ...
  const url = `https://secure.geonames.org/searchJSON?q=${encodeURIComponent(
    query
  )}&maxRows=5&featureClass=P&username=${encodeURIComponent(GEONAMES_USERNAME)}`;
  const res = await fetch(url);
  const data = await res.json();
  if (!data.geonames) return [];
  return data.geonames.map((g) => ({
    name: g.name + (g.countryName ? `, ${g.countryName}` : ""),
...
async function fetchShabbatForMonth(year, month) {
  if (!CAL.cityInfo || !CAL.cityInfo.geonameid) return;
  const first = new Date(year, month, 1);
  const last = new Date(year, month + 1, 0);
  const startStr = dateKey(first);
  const endStr = dateKey(last);
  try {
    const url = `https://www.hebcal.com/shabbat/?cfg=json&geonameid=${CAL.cityInfo.geonameid}&start=${startStr}&end=${endStr}`;
    const res = await fetch(url);
    const data = await res.json();
...
async function fetchHolidaysForYear(year) {
  if (!CAL.cityInfo || !CAL.cityInfo.geonameid) return;
  const isIsrael = CAL.cityInfo.countryCode === "IL";
  const base = `https://www.hebcal.com/hebcal/?v=1&cfg=json&maj=on&min=on&mod=on&year=${year}&c=on&geo=geoname&geonameid=${CAL.cityInfo.geonameid}`;
  const url = isIsrael ? base : base + "&d=1";
  try {
    const res = await fetch(url);
    const data = await res.json();
    const map = {};
...
function createDayCell(d, otherMonth) {
  ...
  if (weekday === 5) {
    ...
    tag.innerHTML = `🕯 <span>כניסת שבת: ${sh?.in || "..."}</span>`;
...
function openDayModal(dateStr) {
  ...
  shInfo.textContent = "";
  const weekday = d.getDay();
  if (weekday === 5 && sh?.in) shInfo.textContent = `🕯 כניסת שבת: ${sh.in}`;
...
async function fetchWeatherForSelectedDay() {
  ...
  try {
    const coords = { lat: CAL.cityInfo.lat, lon: CAL.cityInfo.lon };
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${coords.lat}&longitude=${coords.lon}&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=auto`;
    const res = await fetch(url);
    const data = await res.json();
    ...
    content.innerHTML = `
      <p>מיקום: ${CAL.cityName}</p>
      <p>תאריך: ${selKey}</p>
      <p>טמפרטורה מקסימלית משוערת: <strong>${tMax}°C</strong></p>
      <p>טמפרטורה מינימלית משוערת: <strong>${tMin}°C</strong></p>
      ${
        typeof rainProb === "number"
          ? `<p>סיכוי לגשם: <strong>${rainProb}%</strong></p>`
          : ""
      }
      <p class="muted">נתוני מזג אוויר מ-open-meteo. ניתן להחליף בהמשך ל-API אחר.</p>
    `;
  } catch (e) {
...
function initUI() {
  ...
  document.getElementById("addEventHeaderBtn").addEventListener("click", () =>
    openEventModal(dateKey(CAL.today))
  );
    ...
    try {
      ...
      results.forEach((info) => {
        ...
        div.innerHTML = `
          ...
          <div class="hint">lat: ${info.lat}, lon: ${info.lon}, country: ${info.countryCode}</div>
        `;
